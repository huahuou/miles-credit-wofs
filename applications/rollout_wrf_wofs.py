...
def rollout_case(case_path: str, conf: dict, model: torch.nn.Module, device: torch.device) -> pd.DataFrame:
    ...
    history_len = conf["data"]["history_len"]
    raw_case = dataset.list_upper_ds[0]
    max_steps = len(raw_case["time"]) - history_len
    ...
    y_coord = raw_case["y"].values if "y" in raw_case.coords else raw_case["south_north"].values
    x_coord = raw_case["x"].values if "x" in raw_case.coords else raw_case["west_east"].values
