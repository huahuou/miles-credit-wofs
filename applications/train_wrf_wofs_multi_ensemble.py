...
def primary_main():
    parser = ArgumentParser(description="Train a WoFS multi-step WRF ensemble model with KCRPS support")
    ...
    ensemble_size = int(conf.get("trainer", {}).get("ensemble_size", 1))
    if ensemble_size > 1:
        conf.setdefault("loss", {})
        conf["loss"].setdefault("training_loss", "KCRPS")
        conf["loss"].setdefault("validation_loss", conf["loss"]["training_loss"])

        conf.setdefault("trainer", {})
        if conf["trainer"].get("type", "multi-step-wrf") == "multi-step-wrf":
            conf["trainer"]["type"] = "multi-step-wrf-ensemble"
        conf["trainer"].setdefault("distributed_ensemble", False)

        conf.setdefault("model", {})
        noise_conf = conf["model"].setdefault("noise_injection", {})
        noise_conf.setdefault("activate", True)
