...
def load_dataset_and_sampler(conf, param_interior, param_outside, world_size, rank, is_train=True):
    ...
    logging.info(
        "Loaded WoFS single-step dataset and distributed sampler on rank %s (%s): transforms=%.2fs total=%.2fs",
        rank,
...
def _compute_index_metadata(param_interior: dict, split_name: str, rank: int) -> dict:
    ...
    total_seq_len = history_len + forecast_len
    ...
    for ind_file, filename in enumerate(filenames):
        ...
        try:
            ...
            available = n_time - total_seq_len + 1
            if available > 0:
...
def main(rank, world_size, conf, backend, trial=False):
    ...
    if conf["data"]["scaler_type"] != "std-wrf":
        raise ValueError("WoFS single-step training app currently supports scaler_type='std-wrf' only")
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        drop_last=True,
        prefetch_factor=(1 if distributed_mode else conf["trainer"].get("prefetch_factor", 4)) if thread_workers > 0 else None,
        worker_init_fn=None,
    )
    ...
    if not skip_validation:
        valid_loader = torch.utils.data.DataLoader(
            ...
            drop_last=True,
            prefetch_factor=(1 if distributed_mode else conf["trainer"].get("prefetch_factor", 4)) if valid_thread_workers > 0 else None,
            worker_init_fn=None,
        )
...
def primary_main():
    parser = ArgumentParser(description="Train a WoFS single-step WRF model with same-file t0 conditioning")
