...
SCRIPT="/home/zhanxiang.hua/miles-credit-wofs/applications/rollout_metrics_wrf_wofs.py"
...
if [[ "$MODE" == "none" ]]; then
  echo "Running single-process rollout metrics with config: $CONFIG"
  python "$SCRIPT" "$CONFIG" --mode none "${EXTRA_ARGS[@]}"
elif [[ "$MODE" == "ddp" || "$MODE" == "fsdp" ]]; then
  echo "Running distributed rollout metrics: mode=$MODE nproc=$NPROC backend=$BACKEND config=$CONFIG"
