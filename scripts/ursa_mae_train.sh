#!/bin/bash
#===============================================================================
# Multi-Node Multi-GPU Training Script for WoFS MAE Pre-training on NOAA Ursa
#
...
#   - Scheduler: Slurm
#
# Trains WoFSMultiModalMAE (single-stage multi-modal MAE pre-training).
#
# Usage:
#   sbatch ursa_mae_train.sh
#
# Adjust NUM_NODES, GPUS_PER_NODE, PROJECT, CONFIG, and CONDA_ENV below.
#===============================================================================

#----- Slurm Directives --------------------------------------------------------
#SBATCH --job-name=credit-wofs-mae-train
...
#SBATCH --time=1-11:59:00                       # Wall time limit
...
CONFIG="${PROJECT_DIR}/config/wofs_mae.yml"
...
# Resolve nproc-per-node: user override > auto-detect > GPUS_PER_NODE.
...
#----- Launch MAE Pre-training with torchrun via srun -------------------------
cd ${PROJECT_DIR}
...
echo "MAE training finished with exit code: $?"
