...
class Gather(torch.autograd.Function):
    """Gather tensors across ranks while preserving autograd gradients."""

    @staticmethod
    def forward(ctx, input_tensor):
        ctx.world_size = dist.get_world_size()
        ctx.rank = dist.get_rank()
        gathered = [torch.zeros_like(input_tensor) for _ in range(ctx.world_size)]
...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model: torch.nn.Module, rank: int):
        ...
        logger.info("Loading a multi-step WRF ensemble trainer class")
    ...
    def train_one_epoch(self, epoch, conf, trainloader, optimizer, criterion, scaler, scheduler, metrics):
        ...
        use_distributed_ensemble = bool(conf["trainer"].get("distributed_ensemble", False))
        ...
        if ensemble_size > 1:
            logger.info(
                "Ensemble training enabled with ensemble_size=%d (distributed_ensemble=%s)",
                ensemble_size,
            ...
            for step_index in range(rollout_len):
                ...
                if forecast_step in backprop_on_timestep:
                    ...
                    with autocast(enabled=amp):
                        ...
                        loss = criterion(criterion_target.to(criterion_pred.dtype), criterion_pred).mean()
            ...
            for steps in range(valid_batches_per_epoch):
                ...
                for step_index in range(rollout_len):
                    ...
                    if forecast_step == (forecast_len + 1):
                        ...
                        criterion_pred = y_pred
                        if ensemble_size > 1 and distributed and use_distributed_ensemble:
                            criterion_pred = gather_tensor_no_grad(criterion_pred)
                            criterion_target = gather_tensor_no_grad(criterion_target)
                        loss = criterion(criterion_target.to(criterion_pred.dtype), criterion_pred).mean()
